...
### Регистры стека
1) Sba, Sbb - регистры, указывающие на начало стека.
   - Sba - старший байт.
   - Sbb - младший байт.
2) Spa, Spb - регистры, указывающие на вершину стека.
   - Spa - старший байт.
   - Spb - младший байт.
4) Sc - конфиг стека.
...
Sba, Sbb и Sc следует указывать к начале кода программы и вне основного цикла, если таковой имеется.  
Spa и Spb во время работы менять не рекомендуется, данные регистры управляются процессором.

### Регистры стека вызовов
Стек вызова предназначен для хранения информации о вызове функций командой call и возврат на точку вызова командой ret.
1) Cba, Cbb - регистры, указывающие на начало стека вызовов.
   - Cba - старший байт.
   - Cbb - младший байт.
2) Cpa, Cpb - регистры, указывающие на вершину стека вызовов.
   - Cpa - старший байт.
   - Cpb - младший байт.
4) Cc - конфиг стека.
   - 1-ый бит отвечает за направление стека: 0 - убывает, 1 - возрастает
...
| Тип команды | Тип операции    | Пример записи | Размер (байт) |
|-------------|-----------------|---------------|-------------- |
| 0           | const -> stek   | push 9        | 1 байт        |
| 1           | reg -> stek     | push A        | 1 байт        |

### pop
Id команды: 22. 
| Тип команды | Тип операции | Пример записи | Размер (байт) |
|-------------|--------------|---------------|-------------- |
| 0           | reg <- stek  | pop A         | 1 байт        |
...
| Тип команды | Тип операции               | Пример записи   | Размер (байт) |
|-------------|----------------------------|-----------------|---------------|
| 0           | pc -> cstak & jmp function | call function   | 2 байта       |

### ret
Id команды: 24.  
| Тип команды | Тип операции | Пример записи | Размер (байт) |
|-------------|--------------|---------------|---------------|
| 0           | pc <- cstak  | ret           | 0 байт        |
